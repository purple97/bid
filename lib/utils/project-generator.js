...
var projectGenerator = function projectGenerator(args, callback) {
  ...
  var commonFilePath = _path["default"].join(_path["default"].join(examplesDir, '..'), 'examples/commonFiles');

  if (!args.react) {
    targetPath = _path["default"].join(_path["default"].join(examplesDir, '..'), 'examples/normal');
  } else {
    targetPath = _path["default"].join(_path["default"].join(examplesDir, '..'), 'examples/react');
  }

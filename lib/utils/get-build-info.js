...
const autoGetEntry = (version, devFilePath) => {
  ...
  const getJsEntry = dir => {
    _fs.default.readdirSync(dir).forEach(file => {
      const pathname = _path.default.join(dir, file);
      if (_fs.default.statSync(pathname).isDirectory()) {
        getJsEntry(pathname);
      } else if (jsre.test(pathname)) {
        const relFileName = _path.default.join('./src/', pathname.split(rx_src)[1]);
  ...
  const getJsHtml = dir => {
    ...
    _fs.default.readdirSync(dir).forEach(file => {
      ...
      if (_fs.default.statSync(pathname).isDirectory()) {
        ...
        let relFileName = _path.default.join('./src/', pathname.split(rx_src)[1]);
        ...
        const tmpJS = _path.default.join(_path.default.dirname(relFileName), './index');

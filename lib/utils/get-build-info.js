...
const autoGetEntry = (version, devFilePath) => {
  ...
  const getJsEntry = dir => {
    _fs.default.readdirSync(dir).forEach(file => {
      const pathname = _path.default.posix.join(dir, file);
      if (_fs.default.statSync(pathname).isDirectory()) {
        getJsEntry(pathname);
      } else if (jsre.test(pathname)) {
        const relFileName = _path.default.posix.join('./src/', pathname.split(rx_src)[1]);
...
const autoGetHtml = (version, devFilePath) => {
  ...
  const getJsHtml = dir => {
    ...
    _fs.default.readdirSync(dir).forEach(file => {
      let pathname = _path.default.posix.join(dir, file);
      if (_fs.default.statSync(pathname).isDirectory()) {
        getJsHtml(pathname);
      } else if (htmlre.test(pathname)) {
        let relFileName = _path.default.posix.join('./src/', pathname.split(rx_src)[1]);
        ...
        relFileKey = _path.default.posix.join(_path.default.posix.dirname(_path.default.posix.join(...relFileKeyPaths)), './index');
        const tmpJS = _path.default.posix.join(_path.default.posix.dirname(relFileName), './index');
...
const getBuildInfo = version => {
  let devFilePath = _path.default.posix.join(process.cwd(), './src/p');

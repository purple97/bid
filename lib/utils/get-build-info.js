  ...
  let getJsEntry = dir => {
    _fs.default.readdirSync(dir).forEach(file => {
      ...
      if (_fs.default.statSync(pathname).isDirectory()) {
        ...
        let relFileName = _path.default.join('./src/', pathname.split(/\/src\//)[1]);
        // let v = version ? version + '/' : '';
  ...
  let getJsHtml = dir => {
    ...
    _fs.default.readdirSync(dir).forEach(file => {
      ...
      if (_fs.default.statSync(pathname).isDirectory()) {
        ...
        relFileKey = _path.default.join('src/', relFileKey.split(/\/src\//)[1].replace(htmlre, '/index'));
...
var _default = exports.default = getBuildInfo;
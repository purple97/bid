...
{
  "htmlEntry": [
    "./src/p/react-redux/index.html",
    "./src/p/react-test/index.html"
  ],
  "jsEntry": {
  ...
  "selectedEntry": [
    "src/p/react-redux/4.2.3/index",
    "src/p/react-test/4.2.3/index"
  ]
  //...其他内容
}
```
...
- 开启了 optimization, 分离代码；
  ...
  - 只在构建发布的时候开启分离 功能;
- webpack5 剔除 node 核心库，需要自行引入（Error Message： webpack < 5 used to
  include polyfills for node.js core modules by default.）
  -node-polyfill-webpack-plugin 插件解决
- 【注意】webpack.base.config.js 中增加以下修改，可能会引起
  npm6.x、npm8.x、pnpm7.x 安装的 br-bid 找不到依赖。
...
- (v1.1.4-beta.4)
- 增加 getGitBranchVersion 方法，获取当前分支的版本号；
  getGitBranchVersion()=>Promise<string>
...
- config.json 中增加 js、ts 文件处理时候处理依赖的配置。 解决部分依赖库不转
  es6、es5 的问题。
...
- webpack.{env}.config.js 中, 增加参数 option = {}
  ，会合并到默认配置中（merge(config, option)）
- 增加暴露 webpack-merge 的接口，方便自定义配置（mergeWithCustomize,
  mergeWithRules, unique）
...
- webpack MultiCompiler 方式处理多个页面同时打包
  ...
  - 入口文件 entry， 从{"src/p/1.0.0/index":"src/p/index/index.js"} 给为
    {index:"src/p/index/index.js"}
  - output.path 日常输出路径从 deploy/build 改为 deploy/build/src/p/1.0.0/
  - output.path 线上输出路径从 deploy/javascripts/build 改为
    deploy/javascripts/build/src/p/1.0.0/
  - 更新依赖：webpack-plugin-inline-source@1.0.8
    - 加工 inline 的 js 文件内容， es6 转 es5，压缩
    - webpack-plugin-inline-source 插件的 loader
      “plugins/html-inline-source-loaders.js”
